...
moveBall(tU8 key)
{
  ...
  // erase last position
  lcdRect((tU8)floor(ball.xPos), (tU8)floor(ball.yPos), ball.size, ball.size, BACKGROUND_COLOR);
...
//      lcdRect(round(ball.xPos), round(ball.yPos), ball.size, ball.size, ball.color);
      lcdRect(floor(ball.xPos), floor(ball.yPos), ball.size, ball.size, ball.color);
...
//    convertToDigits(&buf[5],  (tU8)round(ball.xPos));
    convertToDigits(&buf[5],  (tU8)floor(ball.xPos));
//    convertToDigits(&buf[7],  (tU8)round(ball.yPos));
    convertToDigits(&buf[7],  (tU8)floor(ball.yPos));
